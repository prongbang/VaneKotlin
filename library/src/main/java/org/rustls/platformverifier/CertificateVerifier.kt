...
//
// Vendored from rustls-platform-verifier, tag `v/0.7.0`, path
// android/rustls-platform-verifier/src/main/java/org/rustls/platformverifier/CertificateVerifier.kt
// Copyright (c) the rustls-platform-verifier authors. Upstream licenses:
// https://github.com/rustls/rustls-platform-verifier/blob/main/LICENSE-MIT
// https://github.com/rustls/rustls-platform-verifier/blob/main/LICENSE-APACHE
//
// Vendored rather than consumed as an artifact because this component is not
// published to Maven (rustls/rustls-platform-verifier#115) and ships only
// inside the Rust crate, so a coordinate would be unresolvable for anyone
// consuming Vane's AAR.
//
// Local modifications — keep this list current, it is the diff against
// upstream and the basis of the patch offered back:
//   1. `CertPathValidatorException` is mapped by `e.reason` instead of being
//      reported as revocation unconditionally. See the comment at the catch
...
// `verifyCertificateChain` signature, and StatusCode values 0..6 — is defined
// by the Rust crate in src/verification/android.rs and must not drift.
